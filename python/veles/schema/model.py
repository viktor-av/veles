...
class Model(object):
    ...
    def __init__(self, **kwargs):
        ...
        if kwargs:
            raise TypeError(
                "got an unexpected keyword argument {}".format(kwargs.popitem()[0])
            )
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        sys.modules[cls.__module__].__dict__.setdefault("_models", []).append(cls)
    ...
    def load(cls, val):
        ...
        if val:
            raise SchemaError(
                "got unexpected key {} in model {}".format(
    ...
    def generate_header_code(cls, bases=None):
        ...
        builder_cons = []
        for field in cls.fields:
            arg_type = ("{}" if field.cpp_type()[1] else "std::shared_ptr<{}>").format(
                field.cpp_type()[0]
            )
            if field.optional:
                ...
                arg = "const std::pair<bool, {}>& {}".format(arg_type, field.name)
                builder_cons.append(
                    "{}(false, {})".format(field.name, field.cpp_type()[2])
                )
            else:
                fields_code += "  {} {};\n".format(arg_type, field.name)
                arg = "{} {}".format(arg_type, field.name)
                builder_cons.append("{}({})".format(field.name, field.cpp_type()[2]))
...
""".format(fields_code, "".join(setters), cls.cpp_type()[1], ", ".join(builder_cons))
        ...
        if bases:
            ...
            code += "  {}({}) : {}, {} {{}}\n".format(
                ...
                bases_cons,
                ", ".join("{0}({0})".format(field.name) for field in cls.fields),
            )
        else:
            code += "  {}({}) : {} {{}}\n".format(
                cls.cpp_type()[0],
...
""".format(field.name)
                pack_code += """msg["{0}"] = messages::toMsgpackObject(this->{0});
""".format(field.name)
            ...
            """.format(
                ...
                arg,
                (
                    "std::shared_ptr<{}>({{}})".format(field.cpp_type()[0])
...
""".format(
            ...
            "\n".join(
                [
                    'msg["{0}"] = messages::toMsgpackObject(this->{0});'.format(extra)
                    for extra in extra_pack
                ]
            ),
            cls.cpp_type()[0],
        )
...
""".format(field.name)
            conv_func = f"""{arg_type} obj_{field.name};
  fromMsgpackObject(it_{field.name}->second, &obj_{field.name});"""
            if field.optional:
                builder += """{2}
...
""".format(field.name, arg_type, conv_func)
                ...
                builder += f"""  {conv_func}
  b.set_{field.name}(obj_{field.name});
}} else {{
  throw proto::SchemaError("Nonoptional field \
{field.name} not found when unpacking");
}}"""  # .format(field.name, arg_type, conv_func)
...
toMsgpackObject(const std::shared_ptr<{0}>& val) {{
    ...
    def cpp_type(cls):
        ...
        provided)"""
        return cls.__name__, "::".join(cls.__module__.split(".")[:-1] + [cls.__name__])
...
class PolymorphicModel(Model):
    ...
    def __init__(self, **kwargs):
        if self.object_type is None:
            raise TypeError("Can't instantiate class {}".format(type(self)))
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        if not hasattr(cls, "object_types"):
            ...
            if cls.object_type in cls.object_types:
                raise TypeError("object_type {} already used".format(cls.object_type))
    ...
    def load(cls, val):
        ...
        if not issubclass(rcls, cls):
            raise SchemaError("Object type not a subclass of {}".format(cls.__name__))
...
""".format(
    ...
    def generate_base_source_code(cls):
        code = """
std::shared_ptr<{0}> {0}::polymorphicLoad(const msgpack::object& obj) {{
    auto loc_obj = std::make_shared<messages::MsgpackObject>(obj);
    std::shared_ptr<{0}> out;
    fromMsgpackObject(loc_obj, &out);
    return out;
...
""".format(cls.cpp_type()[1])
    ...
    @classmethod
    def generate_source_code(cls, extra_pack=None):
        return super(PolymorphicModel, cls).generate_source_code(["object_type"])
