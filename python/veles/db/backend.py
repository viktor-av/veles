...
class DbBackend:
    ...
    def get(self, id):
        ...
        return Node(
            ...
            pos_end=db_bigint_decode(pos_end),
            tags=tags,
            attr=attr,
            data=data,
            bindata=bindata,
    ...
    def create(self, node, commit=True):
        ...
        """,
            [(raw_id, tag) for tag in node.tags],
        )
        c.executemany(
            """
    ...
    def set_pos(self, id, pos_start, pos_end, commit=True):
        if not isinstance(id, NodeID):
            raise TypeError("node id has wrong type")
        if not isinstance(pos_start, int) and pos_start is not None:
            raise TypeError("pos_start has to be an int")
        if not isinstance(pos_end, int) and pos_end is not None:
        ...
        c.execute(
            ...
            WHERE id = ?
        """,
            (db_bigint_encode(pos_start), db_bigint_encode(pos_end), raw_id),
        )
        if commit:
            self.commit()
