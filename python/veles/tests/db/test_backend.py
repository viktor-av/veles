...
class TestDbBackend(unittest.TestCase):
    ...
    def test_parent(self):
        ...
        id1 = NodeID()
        id2 = NodeID()
        n1 = Node(id=id1, attr={"name": "parent"}, data=set(), tags=set(), bindata={})
        db.create(n1)
        n2 = Node(
            id=id2,
            parent=id1,
    ...
    def test_attr(self):
        ...
        self.assertEqual(n2.attr["bindata"], BinData.from_spaced_hex(12, "123 456"))
    ...
    def test_data(self):
        ...
        self.assertEqual(
            db.get_data(node.id, "bindata"), BinData.from_spaced_hex(12, "123 456")
        )
        self.assertEqual(db.get_data(node.id, "long"), 0x123456789ABCDEF123456789ABCDEF)
        self.assertEqual(
            db.get_data(node.id, "neglong"), -0x123456789ABCDEF123456789ABCDEF
        )
        with self.assertRaises(TypeError):
    ...
    def test_bindata(self):
        ...
        self.assertEqual(db.get_bindata(node.id, "two", start=1, end=2), b"\x34")
        self.assertEqual(db.get_bindata(node.id, "two", start=1, end=6), b"\x34\x56")
        ...
        db.set_bindata(node.id, "three", start=0x123456, data=b"\x33" * 0x789AB)
        n2 = db.get(node.id)
        ...
        self.assertEqual(db.get_bindata(node.id, "three"), correct)
        db.set_bindata(
            node.id, "three", start=0x6789A, data=b"\x44" * 0x789AB, truncate=True
        )
        n2 = db.get(node.id)
        self.assertEqual(n2.bindata["three"], 0x6789A + 0x789AB)
        correct = b"\x11" * 0x12345 + b"\x22" * (0x6789A - 0x12345) + b"\x44" * 0x789AB
        self.assertEqual(db.get_bindata(node.id, "three"), correct)
        self.assertEqual(
            db.get_bindata(node.id, "three", start=0x789AB), b"\x44" * 0x6789A
        )
        self.assertEqual(
